...
function changeContactNew(selobj, id1, url)
{
//	alert("wegewg");
	var selph = selobj.options[selobj.selectedIndex].value;
    var selph_p = selph.split(":");
...
//	var url = "<c:out value="${pageContext.request.contextPath}"/>";
	var cityid = $("#s_thft_city").find(":selected").val().split(":")[0];
//	alert(cityid);
...
// Cart functions
function ui_cart_view(url,view_pan_id,popup_mode)
{
	$.ajax({
        type: "POST",
        url: "/popup",
        data: {"section":view_pan_id},
        dataType: "text",
        success: function(data){
        	if( data != "" )
        	{
