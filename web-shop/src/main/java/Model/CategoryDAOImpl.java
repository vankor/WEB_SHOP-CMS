...
import Model.Category.OrderingRules;
...
//		Statistics g = template.getSessionFactory().getStatistics();
//		System.out.println("Положено в кеш: "+g.getSecondLevelCachePutCount());
//		System.out.println("Взято из кеша: "+g.getSecondLevelCacheHitCount());
		return toList(list);
	}
...
//		System.out.println(currentfilter.getCountquery());
		
		
		
		for(ParamContainer cont:potentialfilter.getParams()){
			System.out.println("АЙДИХИ СВОЙСТВ "+cont.getPropid()+"       "+propid);
			if(cont.getPropid().equals(propid)){
				Value vval = valDAO.getById(val.getId(), false);
				System.out.println("НУЛЬ "+val.getValue());
				val.setValue(vval.getValue());
				cont.addParam(val);
				System.out.println("БЫВШИЙ НУЛЬ "+val.getValue());
			}
		}
		Map <String, List<Integer>> paramvals = new TreeMap<String, List<Integer>>();
		Query q = null;
...
/*		potentialfilter.getParams().add(potential);
		return propertyFilterCount(potentialfilter, ordering);*/
		return (!list.isEmpty())?(Long)list.get(0):null;
	}
