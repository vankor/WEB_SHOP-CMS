...
<c:set var="action" value="${pageContext.request.contextPath}/admin/admincontent/updatedProducer" />


<div style = "width:900px; padding-left:4%;" id = "form">
<h1>Вести производителя для категории ${producer.category.name}:</h1>
<c:if test = "${result == 'success'}">
<div id = "success"><p>Производитель успешно сохранен!</p></div>
</c:if>
<form:form method="post" action = "${action}"  modelAttribute="producer" enctype="multipart/form-data">
<ul>
...
<div id = "sector">
	<c:if test = "${type == 'update'}">
		...
		<li>
			...
			<form:input id = "cat_id" type = "hidden" value = "${producer.category.id}" path="category.id" />
