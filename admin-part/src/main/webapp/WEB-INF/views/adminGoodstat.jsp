...
<c:set var="action" value="${pageContext.request.contextPath}/admin/admincontent/updatedGoodstatus" />


<div style = "width:800px; padding-left:4%;" id = "form">
<h1>Вести статус товара:</h1>
<c:if test = "${result == 'success'}">
<div id = "success"><p>Статус товара успешно сохранен!</p></div>
</c:if>
<form:form method="post" action = "${action}"  modelAttribute="goodstatus" enctype="multipart/form-data">
<ul>
	...
	<c:if test = "${type == 'update'}">
		...
		<div id = "row">
			...
			<div id = "col2"><form:input path="text" /><form:errors path="text" cssClass="error" /></div>
	...
	<li>
		<div id = "row">
			...
			<form:input type = "hidden" path = "thumb" id = "thumb"/>
				<c:choose><c:when test = "${!empty goodstatus.thumb}"><img src="<c:url value="/resources/images${goodstatus.thumb}" />" width="50" alt="${goodstatus.text}"  /></c:when><c:otherwise>Нет аватара   </c:otherwise></c:choose><form:input id = "newthumb" type = "file" text = "Загрузить новый аватар" path="newthumb"/>
